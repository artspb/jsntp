...
public class NtpMessage {
    ...
    /**
     * Constructs a new NtpMessage from an array of bytes (server side).
     */
    public NtpMessage(byte[] array, double _receiveTimestamp) {
        // See the packet format diagram in RFC 2030 for details
        leapIndicator = 0; // haven't problems
        version = (byte) ((array[0] >> 3) & 0x7);
        mode = 4; // server
        stratum = 14;
        pollInterval = array[2]; // ignore
        precision = array[3]; // ignore

        rootDelay = (array[4] * 256.0) + // ignore
                unsignedByteToShort(array[5]) +
                (unsignedByteToShort(array[6]) / 256.0) +
                (unsignedByteToShort(array[7]) / 65536.0);

        rootDispersion = (unsignedByteToShort(array[8]) * 256.0) + // ignore
                unsignedByteToShort(array[9]) +
                (unsignedByteToShort(array[10]) / 256.0) +
                (unsignedByteToShort(array[11]) / 65536.0);

        referenceIdentifier[0] = array[12]; // ignore
        referenceIdentifier[1] = array[13];
        referenceIdentifier[2] = array[14];
        referenceIdentifier[3] = array[15];

        referenceTimestamp = decodeTimestamp(array, 16); // ignore
        originateTimestamp = decodeTimestamp(array, 24); // request sent by client
        receiveTimestamp = _receiveTimestamp; // request received by server
        transmitTimestamp = decodeTimestamp(array, 40); // reply sent by server
    }
    ...
    public NtpMessage() {
        ...
        this.mode = 3;
        this.originateTimestamp = (System.currentTimeMillis() / 1000.0) + 2208988800.0;
    }
